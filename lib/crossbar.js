...
Crossbar.prototype.req = function(args, callback) {
    ...
    function req() {
    	Req(options, function(err, resp, body) {
	        ...
	        if(body && body.status == 'error') error = body;
	        callback(error, data, body);
	    });
    }
};

Crossbar.prototype.version = function(version) {
	var self = this;
	if(version) self.options.version = version;
	return self.options.version;
};

Crossbar.prototype.validate = function(validate) {
	var self = this;
	if(validate || validate == false) self.options.validate = validate;
